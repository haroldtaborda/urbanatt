...
public class ConsultasDinamicasConstans {
	...
	public static final String CONSULTAR_FACTURA_BASE ="SELECT DISTINCT F.ID_FACTURA,F.NUMERO_FACTURA,F.ID_CLIENTE, F.FECHA_CREACION, F.FECHA_FACTURA, F.VALOR_DEUDA, F.VALOR_PAGADO, F.ESTADO, F.VALOR_FACTURA, F.DESCRIPCION, C.NOMBRECOMPLETO, F.TIPO, F.ID_SUCURSAL, S.NOMBRESUCURSAL, F.DESCUENTO, F.VENDEDOR, sysdate-TO_DATE(F.FECHA_FACTURA, 'dd/MM/yyyy'), C.DIRECCION FROM FACTURAS F "+ 
   " INNER JOIN CLIENTES C ON C.NUMID =F.ID_CLIENTE LEFT JOIN SUCURSALES S ON S.ID_SUCURSAL=F.ID_SUCURSAL  WHERE F.ID_FACTURA IS NOT NULL ";
	...
	public static final String CONSULTAR_FACTURA_NOMBRE_CLIENTE = " AND UPPER(C.NOMBRECOMPLETO) LIKE ? ";
	...
	public static final String ORDER_BY_FACTURAS= " ORDER BY F.ID_FACTURA ASC, C.NOMBRECOMPLETO ";
	...
	public static final String CONSULTAR_ABONOS_FROM=" select NUMERO_FACTURA,NUMERO_RECIBO,VALOR_PAGADO,ID_DETALLE,ID_FACTURA,FECHA_CREACION from DETALLE_FACTURA WHERE  ID_FACTURA= ?";
	
	public static final String CONSULTAR_CLIENTE_PADRE=" select  F.ID_CLIENTE_PRECIOS, F.ID_CLIENTE, F.ESTADO, F.FECHA_CREACION, C.NOMBRECOMPLETO FROM PRECIOS_CLIENTE F "
			+ "INNER JOIN CLIENTES C ON C.NUMID =F.ID_CLIENTE"
			+ "WHERE F.ID_CLIENTE= ?";
	
	public static final String CONSULTAR_CLIENTE_PRECIOS_NOMBRE=" select  F.ID_CLIENTE_PRECIOS, F.ID_CLIENTE, F.ESTADO, F.FECHA_CREACION, C.NOMBRECOMPLETO FROM PRECIOS_CLIENTE F "
			+ "INNER JOIN CLIENTES C ON C.NUMID =F.ID_CLIENTE "
			+ "WHERE UPPER(C.NOMBRECOMPLETO) LIKE ? ";
	
	
	public static final String CONSULTAR_CLIENTE_PADRE_S=" select  F.ID_CLIENTE_PRECIOS, F.ID_CLIENTE, F.ESTADO, F.FECHA_CREACION, C.NOMBRECOMPLETO FROM PRECIOS_CLIENTE F "
			+ "INNER JOIN CLIENTES C ON C.NUMID =F.ID_CLIENTE ";
	
	public static final String CONSULTAR_CLIENTE_PRECIOS=" select P.ID_DETALLE,P.ID_CLIENTE_PRECIOS,P.ID_PRODUCTO, P.NOMBRE_PRODUCTO, P.VALOR from PRECIOS_CLIENTE_DETALLES P INNER JOIN PRECIOS_CLIENTE C ON C.ID_CLIENTE_PRECIOS = P.ID_CLIENTE_PRECIOS WHERE C.ID_CLIENTE= ?";
